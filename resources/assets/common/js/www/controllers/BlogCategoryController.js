app.controller('BlogCategoryController', ['$scope', '$rootScope', '$location', '$http', '$compile', '$routeParams', '$filter', 'DTOptionsBuilder', 'DTColumnBuilder', 'Urls', 'BlogCategory', 'Session', 'Notification', 'BASE_API_URL', 'AUTH_EVENTS', 'LOADING_EVENTS', 'OTHER_EVENTS',
    function ($scope, $rootScope, $location, $http, $compile, $routeParams, $filter, DTOptionsBuilder, DTColumnBuilder, Urls, BlogCategory, Session, Notification, BASE_API_URL, AUTH_EVENTS, LOADING_EVENTS, OTHER_EVENTS) {
        var parentOption = {
            ...
            cache: true,
            ajax: {
                url: BASE_API_URL + 'blogCategory/getBlogCategoryByName?token=' + Session.token,
                dataType: 'json',
                delay: 250,
        ...
        $scope.$watch("data.parent_id", function (newValue) {
            if (!angular.isUndefined(newValue) && newValue) {
                BlogCategory.getDetail(newValue)
        ...
        $scope.edit = function (id) {
            $location.path('/blogCategory/edit/' + id);
        };
        ...
        $scope.delete = function (id) {
            swal({
                ...
                BlogCategory.delete(id).success(function (response) {
        ...
        $scope.add = function () {
            $location.url('/blogCategory/add');
        };

        $scope.cancel = function () {
            $location.path('/blogCategory');
        };
        ...
        $scope.create = function () {
            ...
            BlogCategory.create($scope.data)
                .success(function (response) {
                    console.log(response);
        ...
        $scope.update = function () {
            ...
            BlogCategory.update($scope.data)
                .success(function (response) {
                    console.log(response);
        ...
        $scope.blogCategory = function (id) {
            ...
            DTColumnBuilder.newColumn(null).withTitle('Action').notSortable().renderWith(function (data) {
                ...
                if (data.child > 0) {
                    action = '<button id="edit_' + data.id + '" class="btn btn-warning btn-sm" ng-click="edit(' + data.id + ')">' +
                        ...
                        '</button>' +
                        '&nbsp;' +
                        '<button id="blogCategory_' + data.id + '" class="btn btn-primary btn-sm" ng-click="blogCategory(' + data.id + ')">' +
        ...
        $scope.dtOptions = DTOptionsBuilder.newOptions()
            .withOption('ajax', {
                method: 'POST',
                url: BASE_API_URL + 'blogCategory/getAll?token=' + Session.token,
                error: function (xhr, error, thrown) {
                    console.log(xhr);
                    console.log(error);
                    console.log(thrown);
        ...
        $scope.initializeBlogCategory = function () {
            $('#validation-parent').select2(parentOption);
        };
        ...
        if (Urls.getActionUrl($location.absUrl(), 'add')) {
            $scope.getSlug = function (name) {
                ...
                BlogCategory.getBlogCategorySlugByName(name)
                    .success(function (response) {
                        console.log(response);
        ...
        if (Urls.getActionUrl($location.absUrl(), 'edit')) {
            BlogCategory.getDetail($routeParams.id)
                .success(function (response) {
