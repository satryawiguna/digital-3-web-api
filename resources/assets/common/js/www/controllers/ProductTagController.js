app.controller('ProductTagController', ['$scope', '$rootScope', '$location', '$http', '$compile', '$routeParams', '$filter', 'DTOptionsBuilder', 'DTColumnBuilder', 'Urls', 'ProductTag', 'Session', 'Notification', 'BASE_API_URL', 'AUTH_EVENTS', 'LOADING_EVENTS', 'OTHER_EVENTS',
    function ($scope, $rootScope, $location, $http, $compile, $routeParams, $filter, DTOptionsBuilder, DTColumnBuilder, Urls, ProductTag, Session, Notification, BASE_API_URL, AUTH_EVENTS, LOADING_EVENTS, OTHER_EVENTS) {
        $scope.dtInstance = {};
        $scope.data = {};
        ...
        $scope.edit = function (id) {
            $location.path('/productTag/edit/' + id);
        };
        ...
        $scope.delete = function (id) {
            swal({
                ...
                ProductTag.delete(id).success(function (response) {
        ...
        $scope.add = function () {
            $location.url('/productTag/add');
        };

        $scope.cancel = function () {
            $location.path('/productTag');
        };
        ...
        $scope.dtOptions = DTOptionsBuilder.newOptions()
            .withOption('ajax', {
                method: 'POST',
                url: BASE_API_URL + 'productTag/getAll?token=' + Session.token,
                error: function (xhr, error, thrown) {
                    console.log(xhr);
                    console.log(error);
                    console.log(thrown);
        ...
        if (Urls.getActionUrl($location.absUrl(), 'edit')) {
            ProductTag.getDetail($routeParams.id)
                .success(function (response) {
